  ...
  return (
    <EditableSiteShell>
      <main className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8">
        <section className="rounded-[2rem] border border-[var(--editable-border)] bg-white p-6 shadow-[0_18px_45px_rgba(16,23,40,0.08)] sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.24em] text-[var(--slot4-muted-text)]">
                <MessageSquare className="h-4 w-4 text-[var(--slot4-accent)]" /> Local comments
              </p>
              <h1 className="mt-4 text-4xl font-black tracking-[-0.05em] sm:text-5xl">Comments</h1>
        ...
        {visibleComments.length ? (
          <section className="mt-8 grid gap-4">
            {visibleComments.map((item) => (
              <article key={`${item.articleSlug}-${item.id}`} className="rounded-[1.75rem] border border-[var(--editable-border)] bg-white p-5 shadow-[0_12px_32px_rgba(16,23,40,0.06)]">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <p className="font-black text-[var(--slot4-page-text)]">{item.name}</p>
                    <p className="mt-1 text-xs uppercase tracking-[0.18em] text-[var(--slot4-muted-text)]">{formatDate(item.createdAt)}</p>
                  </div>
                  {item.articleSlug ? (
          ...
          <section className="mt-8 rounded-[1.75rem] border border-dashed border-[var(--editable-border)] bg-white/70 p-8 text-center">
        ...
        {filtered.length > COMMENTS_PER_PAGE ? (
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-[1.75rem] border border-[var(--editable-border)] bg-white p-4 text-sm text-[var(--slot4-muted-text)] shadow-[0_12px_30px_rgba(16,23,40,0.06)]">
            <span>
              Page {currentPage} of {totalPages}
            </span>
            <div className="flex gap-2">
              <button type="button" className="rounded-full border border-[var(--editable-border)] bg-white px-4 py-2 font-black disabled:opacity-40" disabled={currentPage <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>
                Previous
              </button>
              <button type="button" className="rounded-full border border-[var(--editable-border)] bg-white px-4 py-2 font-black disabled:opacity-40" disabled={currentPage >= totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>
                Next
              </button>
            </div>
          </div>
